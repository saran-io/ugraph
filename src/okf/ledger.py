...
A knowledge base accumulates material faster than it processes it. `okf status` answers
...
from okf.model import Page, get_typed_edges, iter_pages
from okf.store import read_md
...
def render_table(items: list[Item], limit: Optional[int] = None) -> str:
    ...
    if not items:
        return "  no sources yet — run `okf ingest`"
...
def render_markdown(items: list[Item], config: Config) -> str:
    ...
    lines = [
        ...
        "# Pipeline ledger",
        "",
        f"_Generated by `okf ledger --write` on {now.strftime('%Y-%m-%d %H:%M UTC')}._",
